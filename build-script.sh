...
INTERFACEVIEW=addition-by-two-functions_iv.aadl
grep "version => \"2" addition-by-two-functions_iv.aadl >/dev/null && {
    echo -e "${INFO} Converting interface view from V2 to V1.3"
    TASTE --load-interface-view addition-by-two-functions_iv.aadl --export-interface-view-to-1_3 __iv_1_3.aadl
...
cd "$SKELS" && rm -f manager.zip && zip -r manager manager/* && cd $OLDPWD || exit -1

cd "$SKELS" && rm -f addition.zip && zip -r addition addition/* && cd $OLDPWD || exit -1
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	--subC addition:"$SKELS"/addition.zip \
